...
def render_sets(model, net, opt, epoch: int, test: bool, run_id: str = None):
    ...
    with torch.no_grad():
        ...
        avatarmodel.load(epoch, test=test)
        ...
        if run_id is not None:
            render_path = os.path.join(
                avatarmodel.model_path, "test_free", f"ours_{epoch}_{run_id}"
            )
        else:
            render_path = os.path.join(
                avatarmodel.model_path, "test_free", f"ours_{epoch}"
            )
        ...
        results = []
        if run_id is not None:
            gs_params_path = os.path.join(
                avatarmodel.model_path, "test_free", f"gs_params_{epoch}_{run_id}"
            )
        else:
            gs_params_path = os.path.join(
                avatarmodel.model_path, "test_free", f"gs_params_{epoch}"
            )
        makedirs(gs_params_path, exist_ok=True)
        ...
        for idx, batch_data in enumerate(tqdm(test_loader, desc="Rendering progress")):
            ...
            if model.train_stage == 1:
                (image,) = avatarmodel.render_free_stage1(
                    batch_data, 59400, epoch, idx, run_id
                )
            else:
